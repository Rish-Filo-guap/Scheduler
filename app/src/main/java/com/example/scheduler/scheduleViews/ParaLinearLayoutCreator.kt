...
class ParaLinearLayoutCreator(
    private val context: Context,
    para: Para,
    val parent: ShowBottomFragmentDialogParaInfo
) {
    ...
    init {
        ...
        classroomTextView?.text =
            para.classRoom.substringBefore(" ") + " " + building // Пример текста
        ...
        when (para.typeOfSubject) {
            TypeOfSubject.Lab.typeSub -> typeSubjectTextView?.setTextColor(
                ContextCompat.getColor(
                    context,
                    R.color.labTextColor
                )
            )

            TypeOfSubject.Lek.typeSub -> typeSubjectTextView?.setTextColor(
                ContextCompat.getColor(
                    context,
                    R.color.lekTextColor
                )
            )

            TypeOfSubject.Pra.typeSub -> typeSubjectTextView?.setTextColor(
                ContextCompat.getColor(
                    context,
                    R.color.praTextColor
                )
            )
        }
